...
def payments(request, pk):
    ...
    invoice = Invoices.objects.filter(pk=pk)
    invoice = invoice[0]
    payment_method = PaymentsMethods.objects.all()

    if request.method == 'POST':       
        amount = request.POST.get('amount')
        amount = Decimal(invoice.still_to_pay) - Decimal(amount)
        PM = request.POST.get('PM')
        PM = PaymentsMethods.objects.filter(method=PM)
        PM= PM[0]
        if amount == 0.00:
            Invoices.objects.filter(pk=pk).update(stats='payed', 
            still_to_pay=0, 
            payment_method=PM.pk)
        elif amount > 0.00:
            Invoices.objects.filter(pk=pk).update(
                stats='partial payed', 
                still_to_pay=amount, 
                payment_method=PM.pk)
        else: 
